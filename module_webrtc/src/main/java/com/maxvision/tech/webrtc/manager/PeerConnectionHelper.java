...
import com.maxvision.tech.webrtc.ws.IWebSocket;
...
public class PeerConnectionHelper {
    ...
    public void onJoinToRoom(ArrayList<String> connections, String myId, boolean isAudioEnable, boolean isVideoEnable, int mediaType) {
    ...
    private void closePeerConnection(String connectionId) {
	 Log.i(TAG, "closePeerConnection onCloseWithId:"+connectionId);
    ...
    //判断是插入耳机状态，自动切换耳机模式
    public void autoToggleHeadset() {
        if (mAudioManager != null) {
            toggleHeadset(mAudioManager.isWiredHeadsetOn());
        }
    }

    // 退出房间
    public void exitRoom() {
        Log.i(TAG, "exitRoom: " + (viewCallback == null));
        //退出房间修改  viewCallback = null转移到 onConnectionChange()统一管理viewcallback生命周期
        /*if (viewCallback != null) {
            viewCallback = null;
        }*/
        executor.execute(() -> {
    ...
    private class Peer implements SdpObserver, PeerConnection.Observer {
        ...
        public void onConnectionChange(PeerConnection.PeerConnectionState newState) {
            Log.i(TAG, "onConnectionChange: " + newState.toString() + " " + (viewCallback == null));
            //管理viewCallback生命周期 可监听双方断网状态
            if (newState == PeerConnection.PeerConnectionState.DISCONNECTED || newState == PeerConnection.PeerConnectionState.FAILED || newState == PeerConnection.PeerConnectionState.CLOSED) {
                if (viewCallback == null) {
                    return;
                } else {
        ...
        public void onCreateSuccess(SessionDescription origSdp) {
            Log.i(TAG, "sdp创建成功       " + origSdp.type);
            //设置本地的SDP
        ...
        public void onSetSuccess() {
            Log.i(TAG, "sdp连接成功        " + pc.signalingState().toString());
